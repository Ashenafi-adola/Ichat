...
class CoreConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        self.my_name = self.scope['user'].username
        self.room_name = self.scope['url_route']['kwargs']['friend_name']
        usernames = sorted([self.my_name, self.room_name])
        self.room_group_name = f'friend_{usernames[0]}_{usernames[1]}'
...
class DeleteChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
